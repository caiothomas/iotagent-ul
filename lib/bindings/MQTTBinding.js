...
function recreateSubscriptions(callback) {
    ...
    function subscribeToTopics(topics, callback) {
        ...
        config.getLogger().debug(context, 'topics *', topics);
...
function multipleMeasures(apiKey, device, message) {
    ...
    updates = parsedMessage.reduce(commonBindings.processMeasureGroup.bind(null, device, apiKey), []);
...
function singleMeasure(apiKey, attribute, device, message) {
    ...
    iotAgentLib.update(device.name, device.service, device.subservice, values, device, function(error) {
        if (error) {
            config.getLogger().error(context,
                ' MEASURES-002: Couldn\'t send the updated values to the Context Broker due to an error: %s', error);
        } else {
...
function mqttMessageHandler(topic, message) {
    ...
    config.getLogger().error(context, "mqtt mensage", deviceId, apiKey);
...
function commandHandler(device, attributes, callback) {
    ...
    utils.getEffectiveApiKey(device.service, device.subservice, device.type, function(error, apiKey) {
...
function start(callback) {
    ...
    if (config.getConfig().mqtt && config.getConfig().mqtt.cert && config.getConfig().mqtt.key) {
        options.cert =  fs.readFileSync(config.getConfig().mqtt.cert);
        options.key =  fs.readFileSync(config.getConfig().mqtt.key);
        options.rejectUnauthorized = false;
        config.getLogger().debug(context,'Loading certificates files SSL/TLS MQTT.');        
    }
    
    if(config.getConfig().mqtt && config.getConfig().mqtt.cafile){
        options.cert = fs.readFileSync(config.getConfig().mqtt.cafile); 
    }
